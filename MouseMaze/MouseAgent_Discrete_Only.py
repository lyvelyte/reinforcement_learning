...
class MazeTaskSampler:
    ...
    def restore_hard_grids(
        ...
        """Restore shape-separated schema-v6 replay reservoir state."""
...
class MazeBatch:
    ...
    def observations(self) -> np.ndarray:
        ...
        walls = (self.grids == 1).astype(np.float32)
        agent = np.zeros((self.size, *self.grid_shape), dtype=np.float32)
        indices = np.arange(self.size)
        agent[indices, self.positions[:, 0], self.positions[:, 1]] = 1.0
        goal = np.zeros_like(agent)
        goal[indices, self.goals[:, 0], self.goals[:, 1]] = 1.0
        channels = [walls, agent, goal]
        if self.remaining_time_channel:
        ...
        if self.visit_count_channel:
            channels.append(
                np.minimum(self.visit_counts, self.visit_count_clip).astype(np.float32)
                / self.visit_count_clip
            )
...
class Dashboard:
    ...
    def _poll_loop(self) -> None:
        """Keep the window manager happy without draining the event queue."""
...
def _train_recurrent_ppo(
    ...
    while (
        ...
        effective_entropy_coefficient = (
            config.ppo_entropy_coef + entropy_boost
        ...
        if update_metrics.entropy < config.ppo_entropy_floor:
            entropy_boost = min(entropy_boost + 0.01, 0.1)
        elif entropy_boost > 0:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Resume the selected/latest schema-v6 checkpoint; fresh training "
            "is the default."
        ),
    )
    parser.add_argument(
